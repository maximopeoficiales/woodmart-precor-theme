...
<h1 class="precor-title-email">¡Solicitaste un Presupuesto!</h1>
...
<?php if ((0 !== $customer && (get_option('ywraq_enable_order_creation', 'yes') === 'yes'))) : ?>
	<p><?php printf('%s <a href="%s">%s</a>', esc_html(__('You can see details here:', 'yith-woocommerce-request-a-quote')), esc_url(YITH_YWRAQ_Order_Request()->get_view_order_url($order_id)), esc_html($quote_number)); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
		?></p>
<?php endif ?>
...
?>
			...
			case 'email':
			?>
				<p><strong><?php echo esc_html($field['label']); ?></strong>: <a href="mailto:<?php echo esc_attr(($field['value'])); ?>"><?php echo esc_html($field['value']); ?></a></p>
			<?php
				break;
