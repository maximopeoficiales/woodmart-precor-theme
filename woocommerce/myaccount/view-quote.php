...
//se agrego enlace en nueva pesta�a al plugin yith
if (!empty($actions)) {
...
?>
	...
	<p class="ywraq-buttons">
		...
		if ($print_button_pdf) {
		?>
			<a class="ywraq-big-button ywraq-pdf-file" href="<?php echo esc_url($pdf_file); ?>" target="_blank"><?php esc_html_e('Download PDF', 'yith-woocommerce-request-a-quote'); ?></a><?php } ?>
		<?php
		...
		?>
			<a class="ywraq-big-button ywraq-reject" href="<?php echo esc_url(ywraq_get_rejected_quote_page($order)); ?>"><?php esc_html(ywraq_get_label('reject', true)); ?></a><?php endif ?>
	</p>
...
?>
	...
	if ($print_button_pdf) {
	?>
		<p class="ywraq-buttons"><a class="ywraq-big-button ywraq-pdf-file" href="<?php echo esc_url($pdf_file); ?>" target="_blank"><?php esc_html_e('Download PDF', 'yith-woocommerce-request-a-quote'); ?></a>
		</p><?php } ?>
<?php endif ?>
		...
		?>
				...
				?>
					<tr class="product-purchase-note">
						<td colspan="3"><?php echo wpautop(is_callable('apply_shortcodes') ? apply_shortcodes(wp_kses_post($_product->get_purchase_note())) : do_shortcode(wp_kses_post($_product->get_purchase_note()))); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
										?></td>
					</tr>
		<?php
				endif;
...
<table class="shop_table shop_table_responsive customer_details">
	...
	$billing_address_1 = yit_get_prop($order, 'shipping_address_1', true);
	$billing_address_2 = yit_get_prop($order, 'shipping_address_1', true);
	...
	if ($billing_company) {
		printf('<tr><th>%s</th><td data-title="%s">%s</td></tr>', esc_html__('RUC:', 'yith-woocommerce-request-a-quote'), esc_attr__(
			'RUC:',
